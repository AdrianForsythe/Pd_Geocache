...
run_mantel<-function (...) {
  ...
  visits <- NULL
  for (i in 1:nrow(x)) {
    r <- all_merge[all_merge$year %in% x[i, ]$Var1 & all_merge$county %in% 
                     x[i, ]$Var2, ]
  ...
  incidence <- NULL
  for (i in 1:nrow(y)) {
    r <- all_merge[all_merge$year %in% y[i, ]$Var1 & all_merge$county %in% 
                     y[i, ]$Var2, ]
    r2 <- nrow(r[!is.na(r$wns.map.yr), ])
    incidence <- rbind(incidence, cbind.data.frame(y[i, ], 
                                                   r2))
  }
