...
county_fix<-function (presence.df,counties,can.shape,usa.shape) {
  ...
  can.shape <- can.shape[can.shape@data$PRNAME %in% c("Quebec / Québec", "Ontario", 
                                                      "Prince Edward Island / Île-du-Prince-Édouard", "Nova Scotia / Nouvelle-Écosse", 
  ...
  usa.shape.id <- paste(as.character(sapply(strsplit(usa.shape$names, ","), 
                                            function(x) x[2])), as.character(sapply(strsplit(usa.shape$names, 
                                                                                             ","), function(x) x[1])), sep = ", ")
