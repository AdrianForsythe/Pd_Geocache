...
mapping <- function (...) {
  ...
  ggmap(map) + 
    ...
    geom_point(data = all_summary, aes(y = coords.x2, x = coords.x1, size = total), alpha = 0.25) + 
    theme_classic() + 
  ...
  p <- ggmap(map) + geom_point(data = all_visits_window, aes(y = coords.x2, x = coords.x1, group = User)) + 
