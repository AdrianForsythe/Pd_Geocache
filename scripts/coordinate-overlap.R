  ...
  scraped$year<-year(scraped$Date)
  ...
  presence.scrape <- presence.scrape %>%
    ...
    wns.map.yr = ymd(gsub("-.+", "/01/01", WNS_MAP_YR)),
    r.suspect = ymd(gsub("-.+", "/01/01", YR_SUSPECT)),
    yr.confirm = ymd(gsub("-.+", "/01/01", YR_CONFIRM)))
  ...
  write.csv(presence.scrape,"data/relevant-records.csv")
